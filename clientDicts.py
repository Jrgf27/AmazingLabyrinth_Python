...
def draw_board(board,info):   
    ...
    for i in range(board_length):
        ...
        for j in range(board_length):
            if board[i,j][0]==0:
                draw_piece("straight.png",xposition,yposition,board[i,j][1],info)
            elif board[i,j][0]==1:
                draw_piece("star.png",xposition,yposition,board[i,j][1],info)
            elif board[i,j][0]==2:
                draw_piece("corner.png",xposition,yposition,board[i,j][1],info)
            elif board[i,j][0]==3:
                draw_piece("t_junction.png",xposition,yposition,board[i,j][1],info)
            if board[i,j][3]!=0:
                draw_treasure(xposition,yposition,info)
            if board[i,j][2]!=0:
                draw_players(xposition,yposition,board[i,j][2],info)
            xposition+=1
        yposition+=1
    return 0
                    ...
                    if m.move_player:
                        if event.type == pygame.MOUSEBUTTONDOWN:
                            if pygame.mouse.get_pressed()[0]:
                                ...
                                mouse_y_position=int((pygame.mouse.get_pos()[1]-info.board_pos[0])//(info.square_size*3))
        ####################### Player Position#
                                board_length=int(len(m.board)**0.5)
                                for i in range(board_length):
                                    for j in range(board_length):
